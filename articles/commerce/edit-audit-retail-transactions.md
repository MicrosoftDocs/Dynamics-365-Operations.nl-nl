...
Dynamics 365 Commerce-klanten gebruiken POS-toepassingen (Point of Sale) van dezelfde en andere leveranciers. Met de POS-toepassing van dezelfde leverancier worden winkeltransacties uit de kanalen binnengehaald in Headquarters via een batchproces. Met toepassingen van derden worden transacties binnengehaald in Headquarters via integratie. In beide gevallen moet er nadat transacties zijn binnengehaald in Headquarters, een consistentiecontrole met meerdere validaties op de transacties worden uitgevoerd, zodat het overzicht dat in Headquarters wordt geboekt alleen gevalideerde transacties bevat. 
...
Deze transacties worden gemarkeerd en niet opgenomen in de overzichten, maar kunnen het dagelijkse proces verstoren van het boeken van de dagelijkse verkopen naar de financiële gegevens.

In Commerce kunt u de specifieke detailhandeltransacties die niet worden gevalideerd, bewerken tijdens het controleren van alle wijzigingen. 
...
2. Ga naar het werkgebied **Financiën van winkel**. De tegel **Mislukte transactievalidaties** biedt een vooraf gefilterde weergave van het formulier met de transactie die niet is geslaagd voor een of meer validatieregels.
...
4. In het Excel-bestand wijzigt u de betreffende velden en uploadt u de gegevens weer naar Headquarters met de publicatiefunctionaliteit van de Dynamics Excel-invoegtoepassing. Na de publicatie worden de wijzigingen weergegeven in het systeem. Tijdens het publiceren worden er geen validaties uitgevoerd voor wijzigingen door gebruikers.
...
In Retail versie 10.0.6 en hoger wordt de optie voor het bulksgewijs bewerken van transacties op overzichtsniveau ondersteund. 

---
title: Medewerkers toewijzen aan een verlofplan
description: Werknemers en contractanten toewijzen aan verlofplannen in Dynamics 365 Human Resources.
author: twheeloc
ms.date: 02/03/2020
ms.topic: article
ms.prod: ''
ms.technology: ''
ms.search.form: LeavePlanFormPart, LeaveAbsenceWorkspace
audience: Application User
ms.custom: 7521
ms.assetid: ''
ms.search.region: Global
ms.author: twheeloc
ms.search.validFrom: 2020-02-03
ms.dyn365.ops.version: Human Resources
ms.openlocfilehash: 02baf542b2a9f820385a57d48d2f4519efeb8dc2
ms.sourcegitcommit: 52b7225350daa29b1263d8e29c54ac9e20bcca70
ms.translationtype: HT
ms.contentlocale: nl-NL
ms.lasthandoff: 06/03/2022
ms.locfileid: "8848160"
---
# <a name="assign-workers-to-a-leave-plan"></a>Medewerkers toewijzen aan een verlofplan

> [!Important]
> De functionaliteit die in dit artikel wordt vermeld, is momenteel beschikbaar voor klanten van de zelfstandige versie van Dynamics 365 Human Resources. Sommige of alle functionaliteit is beschikbaar als onderdeel van een toekomstige versie van de Finance-infrastructuur na versie 10.0.26 van Finance.


[!include [Applies to Human Resources](../includes/applies-to-hr.md)]

Nadat u een verlofplan hebt gemaakt in Dynamics 365 Human Resources, wijst u dit toe aan werknemers.

## <a name="assign-a-plan-to-a-group-of-employees"></a>Een plan toewijzen aan een groep werknemers

1. Selecteer op de pagina **Verlof en verzuim** het plan dat u wilt toewijzen.

2. Selecteer **Toewijzen aan werknemers**.

3. Pas de volgende instellingen naar wens aan:

   - **Inschrijven op begindatum dienstbetrekking**
   - **Inschrijvingsdatum**
   - **Einddatum**
   - **Niveaubasis**

4. Selecteer onder **In te schrijven werknemers** alle werknemers die u wilt toewijzen aan het plan.

5. Selecteer **Plan toewijzen** onder aan de pagina.

## <a name="assign-a-plan-to-an-employee"></a>Een plan toewijzen aan een werknemer

1. Selecteer **Verlof** in de record van de werknemer.

2. Selecteer **Verlof en verzuim**.

3. Selecteer **Nieuw**.

4. Voer een inschrijvingsdatum in en selecteer een verlofplan.

## <a name="see-also"></a>Zie ook

- [Overzicht van verlof en verzuim](hr-leave-and-absence-overview.md)
- [Een verlof- en verzuimplan maken](hr-leave-and-absence-plans.md)

[!INCLUDE[footer-include](../includes/footer-banner.md)]

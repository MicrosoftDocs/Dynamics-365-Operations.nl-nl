...
# <a name="cost-accounting-terminology"></a>Terminologie in kostprijsboekhouding
...
- **Vaste kosten** -vaste kosten zijn kosten die niet op korte termijn, ongeacht eventuele wijzigingen in activiteitsniveau verschillen. Vaste kosten kunnen bijvoorbeeld basale bedrijfskosten van een onderneming zijn zoals de huur, die niet worden beïnvloed door schommelingen in het activiteitenniveau.

- **Variabele kosten** -variabele kosten verandert al naar gelang de wijzigingen in activiteitsniveau. Specifieke directe materiaalkosten kunnen bijvoorbeeld zijn gekoppeld aan elk product dat wordt verkocht. Hoe meer producten worden verkocht, des te hoger de directe materialenkosten uitvallen.

- **Half variabele kosten** -half variabele kosten zijn gedeeltelijk vaste en gedeeltelijk variabele kosten. Een internetabonnement kan bijvoorbeeld een maandelijks abonnementsbedrag omvatten plus een tarief voor breedbandgebruik. Het standaard abonnementsgedrag wordt geboekt bij de vaste kosten, maar het tarief voor breedbandgebruik bij variabele kosten.

...
description: Dit artikel worden de berekeningen voor modellen voor productconfiguratie en wordt uitgelegd hoe u berekeningen samen met beperkingen.
author: YuyuScheller
manager: AnnBe
ms.date: 04/04/2017
ms.topic: article
...
Dit artikel worden de berekeningen voor modellen voor productconfiguratie en wordt uitgelegd hoe u berekeningen samen met beperkingen.
...
Het doel voor de berekening wordt de lengte van een netsnoer en de ingevoerde waarde is een kleur in de volgende expressie:  

**Expressie:**\[als kleur == 'Groen', 1,5, 1.0\]  

Wanneer u het artikel configureert, de lengte van het netsnoer is ingesteld op **1,5** als u opgeeft **groen** als de waarde van kleurkenmerk. Als u een andere kleur opgeeft, is de lengte ingesteld op **1,0**. Echter, omdat berekeningen eenrichtings zijn, wordt in de berekening niet de waarde van het kleurenkenmerk op **Groen** ingesteld wanneer u een lengte van **1,5** opgeeft.
...
## <a name="how-do-i-set-a-target-attribute-as-hidden-or-readonly"></a>Hoe stel ik een doelkenmerk verborgen of alleen-lezen?
Om een kenmerk in te stellen als verborgen of alleen-lezen, volgt u deze stappen.

1.  Klik op **productgegevensbeheer**&gt;**veelgebruikte**&gt;**productconfiguratiemodellen**.
...
-   De waarden die in de berekening worden geretourneerd zijn buiten het domein van het kenmerk. Een voorbeeld is een geheel getal van \[1..10\] die wordt berekend op 0.

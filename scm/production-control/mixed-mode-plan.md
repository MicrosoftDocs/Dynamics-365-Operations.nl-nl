---
title: Gemengde modus planning - combineren discrete, verwerken en lean sourcing
description: Dit artikel biedt informatie over de gemengde planmodus. Bij de gemengde planmodus kunt u uw leveringsketen modelleren op basis van de materiaalstroom. Microsoft Dynamics 365 for Operations zorgt ervoor dat de materiaalstroom volgt uw modellen, ongeacht het leveringsbeleid dat is geselecteerd (kanbans, productieorders, inkooporders, batchorders of transferorders).
author: YuyuScheller
manager: AnnBe
ms.date: 04/04/2017
ms.topic: article
...
# <a name="mixed-mode-planning---combine-discrete-process-and-lean-sourcing"></a>Gemengde modus planning - combineren discrete, verwerken en lean sourcing

Dit artikel biedt informatie over de gemengde planmodus. Bij de gemengde planmodus kunt u uw leveringsketen modelleren op basis van de materiaalstroom. Microsoft Dynamics 365 for Operations zorgt ervoor dat de materiaalstroom volgt uw modellen, ongeacht het leveringsbeleid dat is geselecteerd (kanbans, productieorders, inkooporders, batchorders of transferorders). 
...
Dynamics 365 voor de planning van bewerkingen in de gemengde modus bepaalt hoe een product wordt geleverd, en, op basis van de levering, hoe afgeleide behoeften (verbruik van artikelen vanuit een stuklijst van materialen \[stuklijst\]) worden uitgegeven. Op basis van het ordertype vindt het systeem automatisch materialen om aan de vereisten te voldoen.  
...
Ongeacht hoe de keten van toeleveranciers is gemodelleerd, Dynamics 365 for Operations biedt ondersteuning voor uw combinatie van leveringsbeleid. U kunt productieorders hebben die kanbans als bron hebben. Als alternatief kunt u een batchorder hebben die vereist dat een product wordt geleverd door overboekingen of kanbans.  

Dynamics 365 voor bewerkingen zorgt ervoor dat de materiaalstroom het model volgt.  
...
## <a name="materials-allocation-crosssupply-policy--resource-consumption-on-boms"></a>Materialen toewijzing crosssupply beleid: resourceverbruik op stuklijsten
Verbruik van bronnen is een belangrijke functie. Resourceverbruik stelt een magazijn in staat verzamelmaterialen dynamisch te selecteren, op basis van het leveringsbeleid (ordertype) en maakt ook het bijhouden van de basisgegevens eenvoudiger.  
...
Voor werk dat onafhankelijk is van een leveringsbeleid hoeft u geen informatie te wijzigen op de stuklijst als de levering verandert. Voor ad-hoc-wijzigingen zorgt Dynamics 365 for Operations u ervoor dat materialen zijn die afkomstig is van het juiste magazijn.

## <a name="process-manufacturing--the-production-type"></a>Procesfabricage - het productietype
Volledige flexibiliteit in de gemengde modus, wordt u aangeraden productietype stuklijsten te gebruiken voor alle producten. Vervolgens kunt u productieorders, kanbans, transferorders of inkooporders voor het leveren van een product. Voor procesfabricage moet u het productietype **Formule**, **Coproduct**, **Bijproduct** of **Planningsartikel** gebruiken. Kanbans en productieorders kunnen niet voor deze productietypen worden gebruikt.

...
Deze lopend gemiddelde kostprijs wordt voor een artikel berekend met de volgende formule: Geschatte prijs = (Fysiek bedrag + Financieel bedrag) ÷ (Fysieke hoeveelheid + Financiële hoeveelheid)
...
\* Teller = (Fysiek bedrag + Financieel bedrag) \*\* Noemer = (Fysieke hoeveelheid + Financiële hoeveelheid) **Opmerking:** als de optie **Fysieke waarde opnemen** niet is geselecteerd voor een artikel, wordt de waarde 0 (nul) gebruikt voor zowel het fysieke bedrag als het fysieke hoeveelheid. Zie voor informatie over deze optie [Fysieke waarde opnemen](include-physical-value.md).

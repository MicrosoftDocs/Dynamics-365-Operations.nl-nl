...
-   Het label moet met een alfabetisch teken (a tot en met z of A tot en met Z) beginnen en kan elke combinatie van cijfers en letters tot 16 tekens zijn. **opmerking:** een label mag het onderstrepingsteken bevatten (\_), maar geen speciale tekens zijn toegestaan.
...
De cel **Opmaakcode** biedt een selectie van vooraf opgemaakte keuzes voor de inhoud van die rij. Als de cel **Opmaakcode** leeg is, wordt de rij geïnterpreteerd als een rij met financiële gegevens. **Opmerking:** Als een rapport opmaakrijen bevat die geen bedragrij en die zijn gerelateerd aan bedragrijen die zijn onderdrukt (bijvoorbeeld omdat hun saldi nul zijn), kunt u de kolom **Gerelateerde formules/rijen/eenheden** gebruiken om te voorkomen dat titel- en opmaakrijen worden afgedrukt.
...
3.  Selecteer een opmaakcode in de lijst. De volgende tabel beschrijft de opmaakcodes en hun acties.
    ...
    | TOT                           | Totaal                              | Identificeert een rij die wiskundige operatoren gebruikt in de kolom **Gerelateerde formules/rijen/eenheden**. Totalen bevatten eenvoudige operatoren, zoals **+**of**-**.                                                      |
    | CAL                           | Berekening                        | Identificeert een rij die wiskundige operatoren gebruikt in de kolom **Gerelateerde formules/rijen/eenheden**. Berekeningen bevatten complexe operatoren, zoals **+**, **-**, **\***, **/**, en **IF/THEN/ELSE** overzichten. |
...
In de **opmaakcode** kolom in een rijdefinitie is gekoppeld, de **DES**, **LFT**, **RGT**, **CEN**, **---**, en **===**opmaakcodes opmaak toepassen op rijen zonder bedragen. Om te voorkomen dat deze opmaak wordt afgedrukt wanneer de bijbehorende bedragrijen worden onderdrukt (bijvoorbeeld omdat de bedragrijen nulwaarden of geen periodeactiviteit bevatten), moet u de opmaakrijen koppelen aan de bijbehorende bedragrijen. Deze functie is handig wanneer u wilt voorkomen dat koptekst of opmaak die is gekoppeld aan subtotalen wordt afgedrukt als er voor de periode geen details af te drukken zijn. **Opmerking:** U kunt ook voorkomen dat de gedetailleerde bedragrijen worden afgedrukt door de optie te wissen om rijen zonder bedragen weer te geven. Dit optie bevindt zich in het tabblad **Instellingen** van de rapportdefinitie. Standaard worden rekeningen met transactiedetails met een nulsaldo of zonder een periodeactiviteit onderdrukt in rapporten. Om deze rekeningen met transactiedetails weer te geven, selecteert u het selectievakje **Rijen weergeven zonder bedragen** op het tabblad **Instellingen** van de rapportdefinitie.
...
6.  Klik in het menu **Bestand** op **Opslaan als** om de kolomdefinitie onder een andere naam op te slaan. Toevoegen **CBR** aan de huidige bestandsnaam (bijvoorbeeld **CUR\_YTD\_CBR**). Deze kolomdefinitie is de kolomdefinitie van uw basisrij.
...
4.  Voer in de cel **Kolombeperking** de letter in van de rapportkolom die voor het sorteren moet worden gebruikt. **Opmerking:** Neem alleen bedragrijen op in een sorteerberekening.

### <a name="examples-of-ascending-and-descending-column-values"></a>Voorbeelden van oplopende en aflopende kolomwaarden

In het volgende voorbeeld wordt worden de waarden in kolom D van het rapport gesorteerd in oplopende volgorde voor de rijen 160 tot en met 490. Bovendien kunnen worden de absolute waarden in kolom G van het rapport gesorteerd in aflopende volgorde voor de rijen 610 tot en met 940.
...
**Gesorteerd op JTD absolute afwijking in aflopende volgorde**
...
-   **Tekst voor nulwaardevervanging** - De tekst die in het rapport wordt opgenomen wanneer het bedrag 0 (nul) is. Deze tekst verschijnt als laatste regel in het gebied **Voorbeeld**. **Opmerking:** Als de afdruk voor nulwaarden of geen periodeactiviteit wordt onderdrukt, wordt deze tekst onderdrukt.
...
Het type moet de positieve waarde en daarna de negatieve waarde vertegenwoordigen. Doorgaans voert u een vergelijkbare opmaak in die positieve en negatieve waarden onderscheidt. Voer bijvoorbeeld wilt opgeven dat positieve en negatieve waarden twee decimalen hebben, maar negatieve waarden staan tussen haakjes, **0.00;(0.00)**. De volgende tabel geeft aangepaste opmaak weer die u kunt gebruiken om de opmaak van uw waarden te beheren. Alle voorbeelden beginnen vanaf de waarde 1234.56.
...
5.  Volg de stappen onder **Kenmerken** om een invoer toe te voegen voor elk kenmerk dat in de rijcode moeten worden opgenomen:
    1.  Dubbelklik op de cel **Kenmerk** en selecteer een kenmerknaam. **Waarschuwing:** Vervang het hekje (\#) met een numerieke waarde.
...
Wanneer u een specifieke rekening selecteert, combineert de rapportontwerper de rekening en de fiscale jaren, perioden en andere informatie die u in de kolomdefinitie opgeeft. U kunt andere informatie, zoals verschillende boekperioden, gebruiken voor specifieke rijen. De volgende tabel geeft de rekeningmodificators weer die beschikbaar zijn. Vervang het hekje (\#) met een waarde die gelijk is aan of kleiner is dan het aantal perioden in een boekjaar.

| Rekeningmodificator | Wat wordt afgedrukt                                                                           |
|------------------|-------------------------------------------------------------------------------------------|
| /BB              | Het beginsaldo voor een rekening.                                                     |
| /\#              | Het saldo van de opgegeven periode.                                                     |
| /-\#             | Het saldo voor de periode die is \#perioden vóór de huidige periode.                  |
| /+\#             | Het saldo voor de periode die is \#perioden na de huidige periode.                   |
| /C               | Het saldo van de huidige periode.                                                       |
| /C-\#            | Het saldo voor de periode die is \#perioden vóór de huidige periode.                  |
| /C+\#            | Het saldo voor de periode die is \#perioden na de huidige periode.                   |
| /Y               | Het saldo van jaar-tot-datum tot de huidige periode.                                      |
| /Y-\#            | Het saldo van jaar tot heden tot en met de periode die is \#perioden vóór de huidige periode. |
| /Y+\#            | Het saldo van jaar tot heden tot en met de periode die is \#perioden na de huidige periode.  |

### <a name="book-code-modifiers"></a>Boekcodemodificators

U kunt een rij beperken tot een bestaande boekcode. De kolomdefinitie moet ten minste één **FD**-kolom met een boekcode bevatten. **Opmerking:** De boekcodebeperking voor een rij negeert de boekcodebeperkingen in de kolomdefinitie voor die rij.
...
5.  In de **van** veld, de begindimensie invoeren of klik op ![Bladeren](https://i-technet.sec.s-msft.com/dynimg/IC679490.gif "Bladeren") om te zoeken naar beschikbare dimensies. Om een bereik van dimensies in te voeren, voert u in het veld **Tot** de einddimensie in.
...
Wanneer u een natuurlijk segmentwaarde invoert in de **dimensies** in het dialoogvenster kunt u plaatsen een jokerteken teken (?) Of \*) op elke positie van een segment. Rapportontwerper haalt de waarden voor de gedefinieerde posities zonder rekening te houden van de jokertekens. De rijdefinitie bevat bijvoorbeeld alleen natuurlijke segmentwaarden. Natuurlijke segmenten hebben vier tekens. Door te voeren **6???** In een rij, geeft u de instructie rapportontwerper om op te nemen van alle rekeningen die een natuurlijk segmentwaarde hebben die met een 6 begint. Als u **6\***, dezelfde resultaten worden geretourneerd, maar de resultaten ook variabele breedte waarden bevatten, zoals **60** en **600000**. De rapportontwerper vervangt elk jokerteken (?) met het volledig bereik van mogelijke waarden, zoals letters en speciale tekens. Bijvoorbeeld, in het bereik van **12?0** tot en met **12?4** wordt het jokerteken in **12?0** vervangen door de laagste waarde in het tekenreeks en wordt het jokerteken in **12?4** vervangen door de hoogste waarde in de tekenreeks. **Opmerking:** Gebruik jokertekens niet voor de eerste en laatste rekeningen in bereiken. Als u in jokertekens gebruikt in de eerste rekening of de laatste rekening, krijgt u mogelijk onverwachte resultaten.

### <a name="single-segment-or-single-dimension-ranges"></a>Bereiken met één segment of één dimensie

U kunt een bereik opgeven van segmentwaarden of dimensiewaarden. Het voordeel van het opgeven van een bereik is dat u de rijdefinitie niet hoeft bij te werken telkens wanneer een nieuwe segmentwaarde of dimensiewaarde aan de financiële gegevens wordt toegevoegd. Bijvoorbeeld het bereik **+ rekening =\[6100: 6900\]** haalt de waarden van rekeningen 6100 tot en met 6900 naar het rijbedrag. Wanneer een bereik een jokerteken (?) bevat, evalueert de rapportontwerper het bereik niet teken per teken. In plaats daarvan worden de lage en hoge uiteinden van het bereik bepaald en worden vervolgens de eindwaarden en alle waarden daartussen opgenomen. **opmerking:** Report Designer geen rekeningen, dimensies of velden selecteren uit de Microsoft Dynamics ERP-systeem dat geen van de volgende gereserveerde tekens bevatten: &, \*, \[, \], {, of}. U kunt een en-teken (&) alleen toevoegen wanneer u automatisch rijdefinities samenstelt via het dialoogvenster **Rijen invoegen van dimensies**.

### <a name="multiple-segment-or-multiple-dimension-ranges"></a>Bereiken met meerdere segmenten of meerdere dimensies

Wanneer u een bereik invoert met behulp van combinaties van meerdere dimensiewaarden, het bereikvergelijking wordt uitgevoerd op een... \financial-dimensions\dimension-by-Dimension-basis. De bereikvergelijking kan niet teken per teken of op basis van een gedeeltelijke segment worden uitgevoerd. Bijvoorbeeld het bereik **+ rekening =\[5000: 6000\], afdeling =\[1000: 2000\], kostenplaats =\[00\]** geldt alleen voor de accounts die overeenkomen met elk segment. In dit scenario moet de eerste dimensie in het bereik van 5000 tot en met 6000 zijn, moet de tweede dimensie in het bereik van 1000 tot en met 2000 zijn en moet de laatste dimensie 00 zijn. Bijvoorbeeld: **+ rekening =\[5100\], afdeling =\[1100\], kostenplaats =\[01\]** is niet opgenomen in het rapport, omdat het laatste segment dat zich buiten het opgegeven bereik. Als de segmentwaarde van een spaties bevat, zet u die waarde tussen vierkante haken (\[\]). De volgende waarden zijn geldig voor een segment van vier tekens: **\[234\], \[123 \], \[1 34\]**. Dimensiewaarden moeten tussen vierkante haakjes (\[\]), en het rapport wordt deze haken voor u toegevoegd. Wanneer een bereik met meerdere segmenten of meerdere dimensies bevat jokertekens tekens (?) Of \*), het lage en hoge uiteinde van de reeks gehele meerdere segmenten of meerdere dimensies worden bepaald en vervolgens de einde-waarden en alle waarden tussen deze zijn opgenomen. Als u een groot bereik hebt, zoals de hele reeks rekeningen van 40000 tot en met 99999, moet u waar mogelijk een geldige eerste rekening en laatste rekening opgeven. **opmerking:** Report Designer geen rekeningen, dimensies of velden selecteren uit de Microsoft Dynamics ERP-systeem dat geen van de volgende gereserveerde tekens bevatten: &, \*, \[, \], {, of}. U kunt een en-teken (&) alleen toevoegen wanneer u automatisch rijdefinities samenstelt via het dialoogvenster **Rijen invoegen van dimensies**.
...
|-----------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------|
| Voeg twee volledig gekwalificeerd rekeningen toe.                                                       | + Divisie =\[000\], rekening =\[1205\], afdeling =\[00\]+ divisie =\[100\], rekening =\[1205\], afdeling =\[00\] |
| Voeg twee segmentwaarden toe.                                                                 | + Rekening =\[1205\]+ rekening =\[1210\]                                                                           |
| Voeg segmentwaarden toe die jokertekens bevatten.                                    | + Rekening =\[120? + rekening =\[11??\]                                                                             |
| Voeg een bereik van volledig gekwalificeerd rekeningen toe.                                                | + Divisie =\[000: 100\], rekening =\[1205\], afdeling =\[00\]                                                   |
| Voeg een bereik van segmentwaarden toe.                                                          | + Rekening =\[1200:1205\]                                                                                       |
| Voeg een bereik van segmentwaarden toe die jokertekens bevatten.                         | + Rekening =\[120?: 130?\]                                                                                       |
| Trek één volledig gekwalificeerde rekening af van een andere volledig gekwalificeerde rekening.              | + Divisie =\[000\], rekening =\[1205\], afdeling =\[00\]-divisie =\[100\], rekening =\[1205\], afdeling =\[00\] |
| Trek één segmentwaarde van een andere segmentwaarde af.                                  | + Rekening =\[1205\]-rekening =\[1210\]                                                                           |
| Trek een segmentwaarde af die een jokerteken van een andere segmentwaarde bevat. | + Rekening =\[1200\]-rekening =\[11??\]                                                                           |
| Trek een bereik van volledig gekwalificeerd rekeningen af.                                           | -Divisie =\[000: 100\], rekening =\[1200:1205\], afdeling =\[00:01\]                                           |
| Trek een bereik van segmentwaarden af.                                                     | -Rekening =\[1200:1205\]                                                                                       |
| Trek een bereik van segmentwaarden af die jokertekens bevatten.                    | -Rekening =\[120?: 130?\]                                                                                       |

Hoewel u de rekeningen rechtstreeks kunt wijzigen, kunt u ook het dialoogvenster **Dimensies** gebruiken om de juiste opmaak toe te passen op uw koppelingen naar financiële gegevens. Alle waarden kan bevatten jokertekens tekens (?) Or \*). Echter, Report Designer rekeningen, dimensies of velden niet selecteren uit de Microsoft Dynamics ERP-systeem dat geen van de volgende gereserveerde tekens bevatten: &, \*, \[, \], {, of}. **Opmerking:** Om waarden af te trekken, moet u deze waarden tussen haakjes plaatsen. Als u bijvoorbeeld **450?-(4509)**, wordt weergegeven als **+ rekening =\[4509\]-rekening =\[450? \]**, en u bent geeft u de opdracht rapportontwerper om af te trekken van het bedrag van rekeningsegment 4509 van het bedrag van elk rekeningsegment dat met 450 begint.
...
3.  Volg in de eerste rij van het dialoogvenster **Dimensies** deze stappen:
    ...
    2.  Dubbelklik op de **Operator +/-** cel en selecteert u het plusteken (**+**) of min (**-**) operator op die van toepassing op een of meer dimensiewaarden of groepen in de rij.
...
**Opmerking:** De operator is van toepassing op alle dimensies in de rij.
...
|---------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Persoon of bereik | In de **van** veld, voer de naam van een rekening of klik op de **Bladeren** knop ![Bladeren](https://i-technet.sec.s-msft.com/dynimg/IC679490.gif "Bladeren") om te bladeren naar de rekening. Om bereik te selecteren, kunt u in het veld **Tot** een waarde typen of zoeken.                                             |
| Set van dimensiewaarden | Geef in het veld **Naam** de naam van een set van dimensiewaarden op. Om een set te maken, wijzigen, kopiëren of verwijderen, klikt u op **Sets van dimensiewaarden beheren**. De **formule** veld wordt gevuld met de formule van de **koppeling naar financiële dimensies** cel voor deze dimensiewaarde die is ingesteld in de rijdefinitie vermeld. |
...
5.  Open het dialoogvenster **Wijzigen** en wijzig de formulewaarden die in de set moeten worden opgenomen. **Opmerking:** Als u nieuwe rekeningen of dimensies toevoegt, moet u de bestaande sets van dimensiewaarden wijzigen om de wijzigingen op te nemen.
